...
(function(){
  ...
  BM._projectDecision=async function(p,ctx,topObligations,fundingRows){
    const base=await _proj141(p,ctx,topObligations,fundingRows),route=base.publicRoute||this._projectPublicRoute(p,ctx),effort=base.effort||this._projectEffort(p,ctx),logic=await this.simpleLogic(),cfg=logic.author_support||{},specialist=this._specialistProject(p),priority=['do_now','prepare','check_existing'].includes(base.decision),enoughEffort=effort.points>=(cfg.minimum_effort_without_specialism||5),needHelp=priority&&(enoughEffort||specialist);
    let externalSupport={show:false,mode:'none',reason:'',caution:cfg.caution||'Confirmar primero servicios públicos disponibles.'};
    if(needHelp&&['legal_check','reconfirm'].includes(route.level))externalSupport={show:true,mode:'after_public_check',reason:'Existe una función o antecedente provincial relacionado, pero Brújula no ha verificado que cubra realmente esta actuación. Compruébalo primero; si no la cubre, puede tener sentido pedir apoyo especializado.',caution:cfg.caution};
    ...
    else if(needHelp&&ctx?.entityRoute!=='direct'&&route.level!=='verified'&&route.level!=='common')externalSupport={show:true,mode:'after_parent_check',reason:`Primero acuerda con ${ctx.admin?.name||'el municipio matriz'} la vía de ejecución. Si no existe apoyo supramunicipal suficiente, puede ser útil apoyo especializado.`,caution:cfg.caution};
